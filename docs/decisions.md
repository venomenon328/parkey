...
| ID | Entscheidung |
| --- | --- |
| D-019 | Bereits betretene Felder und aktuell erreichbare Nachbarfelder haben einen klar erkennbaren visuellen Status gegenüber dem Standard. Das aktuelle Feld bleibt eindeutig. Besuchsstatus und Erreichbarkeit sind kombinierbar; ein besuchter Nachbar bleibt als erreichbar erkennbar. |
...
| D-022 | Statusfarben bleiben Varianten der jeweiligen Grundfarbe eines Tiles: besucht dunkler, erreichbar etwas heller als Standard. Keine eigenständige bunte Statuspalette oder vollständiger Farbtonwechsel. Bei besucht+erreichbar hat die helle Erreichbarkeitsdarstellung Vorrang; eine dezente Formmarkierung erhält den Besuchshinweis. Aktuell durch dezenten Rand/Positionsmarker. |
...
D-019/D-022 sind Darstellungszustände und verändern weder Streckendaten noch -identität. Der Besuchsstatus folgt akzeptierten logischen Feldwechseln je Versuch; Start ist besucht, Rückwege löschen Historie nicht, Quick Restart setzt auf den neuen Startzustand zurück.
...
| P-006 | Lokale Ranglisten je Strecken-/Regelidentität als vorbereiteter P1c-Auftrag, noch nicht implementiert | [P1c-Vertrag](p1c-local-results.md); keine automatische Windows-/Web-Synchronisierung. Gleichstands-/Aufbewahrungsparameter sind vorläufige Arbeitsfestlegungen dieser Vorbereitung, keine neue ausdrückliche Produktentscheidung. |
| P-007 | Automatische Third-Person-Rückkamera mit lesbaren Nachbarn und rechtzeitig sichtbaren Verzweigungen | Kontinuität ist abgenommen; Kamera-/Beschriftungsfeinschliff und D-023/D-024 folgen in P2a. |
...
**P1b / Issue #3:** Abgeschlossen und über PR #13 gemergt (`e8e947e4100c8f3e534ae425752ac2c30c7fee7a`). `integration` 189 Assertions, `all` 350; beide Exporte, PR-CI und `main`-CI erfolgreich. Die physische/manuelle **Windows-Abnahme ist bestanden**. Wegen D-008 wurde die noch offene physische Chrome-Nutzerabnahme nach ausdrücklicher Nutzerentscheidung auf später verschoben und war kein P1b-Mergeblocker. Sie darf nicht als bereits bestanden dargestellt werden. D-023/D-024 dokumentieren die bewusst verbleibende Kamera-/Beschriftungsfolgearbeit für P2a.

**P1c / Issue #4:** Zur Umsetzung vorbereitet auf `codex/p1c-local-leaderboards`, noch nicht implementiert. Der [Paketvertrag](p1c-local-results.md) legt als vorläufigen Arbeitsstand numerische Mikrosekundensortierung, gemeinsame Ränge bei exakter Zeitgleichheit, Top 100 je vollständiger Identität und eine Top-10-Ergebnisansicht fest. Speicherung verändert weder Originalzeit noch Regel-/Streckenidentität. Keine neue ausdrückliche Nutzerfreigabe dieser Detailparameter behauptet.

**Vor P2/P3:** Kamera-/Routenlesbarkeit und D-023/D-024 in #5 erproben; Zielhardware, Auflösung und Leistungsbudget für #6 klären. Nur geeignete Bausteine für #7 freigeben. Weitere Tastaturlayouts, endgültige Streckenlängen, Größenbereiche und Anschlussgrenzen bleiben zu erproben. Höhenwechsel, Sprung-/Brückenmechaniken und beliebige Polygon-Generierung sind nicht mitbeauftragt.
