...
`RouteMeasurement` speichert nur während eines laufenden Versuchs im Arbeitsspeicher: gewählte Abschnitts-ID, Abschnittsdauer auf derselben empfangenen monotonen Zeitbasis und Fehlerzahl des Abschnitts. Es schreibt keine Datei, blockiert keine Eingabe und sendet keine Daten. Die Resultatansicht kann abgeschlossene Abschnittsproben dieses einen Laufs anzeigen.

## QWERTZ-Hypothesen und Testbericht

`QwertzTypingHypotheses` legt für die verwendeten Buchstaben offen Hand, Finger und Reihe der beschrifteten QWERTZ-Tasten ab. Daraus werden nur Eingabeschritte, Finger-/Hand- und Reihenwechsel gezählt. `ASDFGH` und `QWERT` sind als **Hypothesen bekannter Folgen** markiert; `FJK` als kurze Folge mit Handwechseln und `PLM` als kurze Folge mit Reihenwechseln. Diese Beschreibungen sagen nicht voraus, welche Route für eine Person schneller, angenehmer oder ergonomisch besser ist.

Der kontrollierte Test deckt alle vier Kombinationen der beiden Entscheidungen ab. Er misst beispielsweise `alpha_short_fjk` mit einem absichtlich erzeugten Fehler und prüft die Abschnittszeit inklusive der vorhandenen 200-ms-Fehlerpause. Damit sind Zeit, Tippfehler und Entscheidungsweg getrennt auswertbar; Entscheidungszeit und Sichtprobleme benötigen weiterhin reale Spielbeobachtungen.
...
Automatisierte Nachweise prüfen Graph, Layout, Ports, Anschlüsse, alle vier Routen, Identität, Kamera-Vorschau, Oberflächenbeschriftung, QWERTZ-Beschreibungen sowie die in-memory Messung. Der tatsächliche Windows-Release wurde auf Windows 11 Pro Build 26200 bei 2560 × 1440 sichtbar gestartet und an Bereitschaft sowie erster Entscheidung nach synthetischem `A → Z → K` geprüft. Die gespeicherten, unversionierten Screenshots zeigen die Keycap-Beschriftungen ohne Callouts und beide ersten Alternativen vor der Wahl. Dieser technische Sichtnachweis ersetzt keine menschliche Spielabnahme. Der Web-Release wurde über HTTP bereitgestellt, eine isolierte grafische Browserprüfung war in der Ausführungsumgebung nicht startbar und bleibt offen.
