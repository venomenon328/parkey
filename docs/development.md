...
Die Nacharbeit erfasst acht Zustände einschließlich `beta` und `beta_long` für die schmalen Formate und das breite W. Native Messfenster sichtbar öffnen und während des Laufs fokussiert lassen; verdeckte Fenster können vom Desktop gedrosselt werden und liefern keine vergleichbare Vordergrundleistung. Der erste native Frame protokolliert zusätzlich die gemeldete Bildschirmfrequenz. `.gdignore` hält die reine Gestaltungsreferenz ebenso wie die Renderbelege aus dem Spielimport heraus.
