...
| Anzeige | Seit der ausdrücklich bestätigten P2b-Nacharbeit aus #6 ausschließlich `MM:SS.mmm` in Ziel-Overlay, Bestzeit und Top 10, ganzzahlig abgeschnitten. Mikrosekunden bleiben intern und in F3/Evidenz verfügbar; unterschiedlich gerankte Originalzeiten dürfen dieselbe Millisekundenanzeige haben. Keine Rohzeit überschreiben; Rang und exakte Gleichstandslogik bleiben unverändert. |
...
Der Implementierungsstand erfüllt die automatisierten Abnahmetabellen mit `storage` (67 Assertions), `integration` (218) und `all` (446). Der Release-Nachweis auf Windows 11 Pro / Godot 4.7.2 umfasst einen nativen Abschluss und Neustart mit unveränderter Datei. Der Chrome-Release lief über HTTP unter gleicher Origin mit zwei erhaltenen unterschiedlichen IDs über Reload sowie Tab-Schließen/Neuöffnen. Eine Chrome-Konfiguration mit blockierter Site-Datenspeicherung ergab den erwarteten IndexedDB-Zugriffsfehler und keinen dauerhaften Ergebniseintrag; die temporäre Resultatmeldung ist zusätzlich in der echten Szenenintegration abgedeckt. Die neue Storage-Regression deckt ausschließlich vorhandene `.bak` → Recovery → Ersetzungsfehler → frischen Reload ab und erhält dabei exakt den alten Stand. Die historische P1c-Szenenregression führte zwei echte Viewport-Läufe mit `1.234.000` und `1.234.999` µs aus: beide zeigen `00:01.234`, der abweichende Rang bleibt erhalten. Seit der ausdrücklichen P2b-Nacharbeit prüfen diese Regressionen mikrosekundenfreie normale Tabellenzellen und den exakten F3-Nachweis; der interne Speicher-/Gleichstandsvertrag bleibt unverändert. Details der visuellen Browserprüfung stehen in [testing.md](testing.md).
