...
`RunnerVisual` verbindet Kopf, Rumpf, Kleidung und Gliedmaßen und mischt Idle/Bewegung/Reaktion. Die bestehende Szenensteuerung besitzt weiterhin allein die begrenzten Wegpunkte. Die Rückkamera verwendet präsentationsseitig Höhe 4,7, Abstand 7,4, normalen Vorblick 3,4 und einen seitlichen Schulterversatz von 1,0; an Entscheidungen bleibt der vorhandene Mittelpunkt beider Vorschauabschnitte maßgeblich. 50-/80-ms-Aufholbudgets und P1-Kernzeiten bleiben unverändert. Alle 30 Kamerapositionen werden mit vollständigen projizierten Glyphen des aktuellen Felds und aller direkten Nachbarn geprüft; tatsächliche Renderbilder prüfen zusätzlich Verdeckung und Bildwirkung.

`WorkshopWorld` baut dieselbe Werkstatt für beide Profile einschließlich eines texturierten Kulissenbodens unter den Seitenstationen und auf den Schrankplatten stehender Pflanzen. Zusätzliche freie Start-/Zielschrift und Boden-Schriftzüge entfallen; nur das eigentliche Portal ist beschriftet. Der Boden besitzt keine Kollision oder Kernverbindung. Unbewegliche Möbel werden nach Material zu Meshes gebündelt, unabhängig bewegte Keycaps bleiben einzelne Knoten. Barlow Medium und Barlow Semi Condensed SemiBold sind versionierte OFL-Ressourcen. Das versionierte CC0-HDR ersetzt die Laufzeit-Wolkenberechnung; `atelier_sky.gdshader` tastet eine angehobene Wolkenhemisphäre für die schwebende Welt ab und konvertiert nur im Compatibility-Profil die lineare HDR-Ausgabe nach sRGB. Keine zusätzliche Bibliothek und kein Netzwerk zur Laufzeit.

`AtelierMesh` liefert kosmetische gerundete Vollkörper, gedrehte Profile und gebogene Röhren. Figur und Werkstatt verwenden diese gemeinsame Modellierung; sie erzeugt keine Kursdaten. Jacke, Frisur, Hände, Schuhsohlen/Nähte, geformte Pflanzen, Schubladen/Beschläge und Portalgliederung ersetzen die einfacheren Formen. Identische unveränderliche Grundmeshes werden geteilt, ebenso die deindizierten Zwischenmeshes beim Aufbau; das reduziert wiederholte Generierung ohne Geometrieabbau. Statische Geometrie wird nach Materialressource gebündelt, damit Texturen und Shader beim Zusammenführen erhalten bleiben.

Forward+ verwendet 4× MSAA, einen weich gefilterten Schattenwerfer (Winkel 2,5°, Distanz 55), schattenloses Fülllicht, ACES, SSAO (Radius 0,65, Intensität 1,65) und zurückhaltendes SSIL (Radius 2, Intensität 0,65). Compatibility erhält dieselbe Geometrie, Schriften, Material-/Hubsignale und den Himmel ohne Schatten/SSAO/SSIL/MSAA und mit linearem Tonemapping. Kein Glow, DOF, SSR, volumetrischer Nebel oder Partikelsystem. Kunststoff-/Kleidungsnormalen und ein entfernungsgefilterter Mattenshader ergänzen lokal versioniertes CC0-Fotoholz mit Farb-, Normal- und Rauheitskanal. Die Holzabbildung folgt der Brettlänge; Dielenfugen bleiben reine Materialdetails.
