...
`KeycapVisual` modelliert eine glatte verjüngte Schale mit Schulter/Mulde innerhalb unveränderter P2a-Grundflächen. Die Normalen entstehen vor der Materialtrennung am unteren Sockelrand gemeinsam; keine sichtbare Drei-Ring-Schichtung. Oberseite und integrierte Barlow-Legende folgen ausschließlich dem visuellen Hub. Statussymbole entfallen gemäß der neuesten #6-Entscheidung. Grundfarbrelative Materialien, Figur und Hub erhalten `besucht > erreichbar`; die kanonischen Transforms, Anker und Transitionen bleiben unverändert.

`RunnerVisual` verbindet Kopf, Rumpf, Kleidung und Gliedmaßen und mischt Idle/Bewegung/Reaktion. Die bestehende Szenensteuerung besitzt weiterhin allein die begrenzten Wegpunkte. Die Rückkamera verwendet präsentationsseitig Höhe 4,2, Abstand 6,8, normalen Vorblick 3,4 und einen seitlichen Schulterversatz von 1,0; an Entscheidungen bleibt der vorhandene Mittelpunkt beider Vorschauabschnitte maßgeblich. 50-/80-ms-Aufholbudgets und P1-Kernzeiten bleiben unverändert. Alle 30 Kamerapositionen werden mit vollständigen projizierten Glyphen des aktuellen Felds und aller direkten Nachbarn geprüft; tatsächliche Renderbilder prüfen zusätzlich Verdeckung und Bildwirkung.

`WorkshopWorld` baut dieselbe Tastatur-Werkstatt für beide Profile. Drei asymmetrische Arbeitsplätze zeigen Switch-Montage, Keycap-Farb-/Profilproben und Tastaturprüfung; eine gemeinsame Materialpalette, Versorgungsleitung, Teilelager und seitliche Holzüberdachung verbinden das Set. Mobiliar und Requisiten bleiben außerhalb der kanonischen Feldhülle. Der zusammenhängende texturierte Kulissenboden liegt unter der Strecke und besitzt keine Kollision oder Kernverbindung. Stationsschilder sitzen auf Möbeln; nur das eigentliche Zielportal trägt Zielschrift.

Stationsarbeitsplatten verwenden eine eigene prozedurale helle Leimholztextur (`workshop_worktop.gdshader`): feine längsgerichtete Fasern, dezente Lamellenvariation und seidenmatte Rauheit. Sie teilen weder die Fototextur noch die groben Dielenfugen des Bodens. Der Shader arbeitet in Werkstattkoordinaten nach der statischen Bündelung und filtert feine Fasern mit der Entfernung; beide Renderer verwenden dieselbe Materialdefinition.

`AtelierMesh` liefert ausschließlich kosmetische gerundete Körper, Profile und Röhren. Unveränderliche Grundmeshes werden geteilt; die statische Werkstatt wird nach Materialressource gebündelt, bewegte Keycaps und Figur bleiben getrennt. Die Figur besitzt einen zusammenhängenden modellierten Bob, kleine runde Haarformen, runden Kopf, Hoodie mit eigener Stickerei, kurze Beine und mehrteilige helle Schuhe. Gliedmaßen-/Kopfzustände und Bewegungsfristen bleiben beim bestehenden Controller.

Forward+ nutzt 4× MSAA, einen weichen Schattenwerfer (Winkel 3,5°, Distanz 55), schattenloses Fülllicht, ACES, SSAO (Radius 0,65, Intensität 1,5) und zurückhaltendes SSIL (Radius 2, Intensität 0,65). Drei kleine schattenlose Arbeitsleuchten setzen lokale warme Akzente. Compatibility erhält dieselben Modelle, Schriften, Material-/Hubsignale und das Wolkenpanorama ohne Schatten/SSAO/SSIL/MSAA und mit linearem Tonemapping. Kein Glow, DOF, SSR, volumetrischer Nebel oder Partikelsystem. Feiner dosierte Kunststoff-/Kleidungsnormalen, eine entfernungsgefilterte Matte und CC0-Fotoholz mit Farb-, Normal- und Rauheitskanal differenzieren die Oberflächen. Holzabbildung folgt Brettern, Querstreben und senkrechten Pfosten; Dielenfugen bleiben Materialdetails. `atelier_sky.gdshader` stimmt den klaren Himmel auf Cyan ab und berücksichtigt den Compatibility-Farbraum. Keine Laufzeitdownloads, neue Bibliothek oder Fremdmodelle.

Windows initialisiert das Fenster mit FIFO (`window/vsync/vsync_mode.windows=1`); `WindowPacing` schaltet anschließend am bestehenden Fenster auf Mailbox-VSync. Auf der geprüften NVIDIA-/Windows-Konfiguration ergab diese Reihenfolge einen anderen Present-Pfad als ein unmittelbarer Mailbox-Start. Der Knoten begrenzt auf die aufgerundete gemeldete Frequenz des aktuellen Fenstermonitors, bei fehlenden Daten auf 60 FPS. Er prüft Monitorwechsel alle 0,5 Sekunden, respektiert explizite bestehende FPS-Limits und gibt sein Limit beim Verlassen der Szene frei. Er steuert weder Fokus/Fensterposition noch Eingaben, Laufuhr oder Animationstermine. Web bleibt browsergetaktet. Die Strategie behebt die reproduzierten ungleichmäßigen Anwendungsframezeiten; die Grenzen zusätzlicher Desktop-/Present-Zeitstempel stehen neben den realen Releasebelegen im Paketbericht.

`WorkshopHUD` trennt Timer/Fehler/Ergebnis/Top 10 von F3. Standardzeiten verwenden ausschließlich `MM:SS.mmm`; Roh-Mikrosekunden bleiben in Speicher/Sortierung und F3. Eigene Tabellenzeilen ordnen Rang, Zeit und Fehlerzahl; aktuelle Zeile und Ergebnisse außerhalb der Aufbewahrung bleiben kenntlich. Ein voller Ergebnisschirm wird gegen die Viewportgrenze geprüft.

`tests/render_evidence.gd` bleibt ausschließlich über `--p2b-evidence` bzw. `?evidence=1` aktiv. Synthetische Viewport-Eingaben laufen in der echten exportierten Szene mit realer Uhr und isolierter Testablage. Der Helfer protokolliert zusätzlich Frames ohne Fensterfokus. Normales Spielen instanziiert ihn nicht. Details, Quellen und Grenzen: [P2b-Bericht](p2b-visual-slice.md).
