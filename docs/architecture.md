...
Die visuelle Nacharbeit korrigiert die Seiten-Wicklungsrichtung der prozeduralen Körper. Kappenschale und Statusoberfläche teilen eine geschlossene Naht; Fasen und Mulde bleiben innerhalb der kanonischen Grundfläche. `Label3D` nutzt beleuchtete Druckfarbe ohne Outline und formatabhängige Schriftmetriken; die tatsächlichen gedrehten Glyphen-AABBs werden regressionsgeprüft. Der Himmel wird einmalig als 1024×512-Panorama aus deterministischem sphärischem Rauschen aufgebaut und innerhalb des Prozesses wiederverwendet; Distanznebel übermalt den Himmel nicht. Kein laufender Noise-Shader oder neuer Renderpass. Der mittige Timer und `stopwatch_icon.gd` verwenden dieselbe skalierende HUD-Leinwand.
