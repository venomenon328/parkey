...
Der weitere Fidelity-Pass ergänzt Prüfungen für geformte Möbel nach Materialbündelung (reale Vertex-/Normaldaten), Charakterzusammenbau, beide SSIL-Profile und die monitorabhängige FPS-Begrenzung einschließlich ungültiger Frequenzdaten. Endgültiger Headless-Stand am 2026-09-06: `presentation` 539, `integration` 238 und `all` 1089 Assertions ohne Fehler; Import und beide Releaseexporte Exit 0, fehlende/unbekannte Suite Exit 1. Diese Prüfungen allein bewerten weder Materialausgabe noch Present-Pacing.

Vier neue reale 1440p-Iterationen gegen den externen Mock und `5659242` korrigieren unter anderem Holzfinish, Frisur, Materialfilterung und längsgerichtete Maserung. Finale Windows-1080p-/1440p- und Chrome-Belege sowie gesonderte native PresentMon-ETW-Messungen stehen im [P2b-Bericht](p2b-visual-slice.md#versionierte-belege). Default-Messungen verwenden keine VSync-/FPS-Diagnoseflags. Die alte FIFO-Diagnose wird getrennt vom neuen normalen Mailbox-/Monitorlimit-Nachweis ausgewertet; beide angeschlossenen Monitorfrequenzen werden zusätzlich geprüft. ETW-Bildwechsel sind kein optischer Scanout-/Eingabelatenztest. Die subjektive Abnahme bleibt offen.

Der Nutzer verwendet ausdrücklich beide Monitore (144 Hz und 60 Hz); die normale Konfiguration muss auf beiden mit echten Release-/Present-Daten validiert werden. Gleichmäßige Engine-Ticks allein schließen ungleichmäßige Bildausgabe im Windows-Compositor nicht aus.
