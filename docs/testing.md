...
Stand: 2026-09-06. P0/P1 sind abgenommen und gemergt. P1b umfasst 189 historische Integrations- und 350 Gesamtassertions; finale PR-CI `34001773894` und Merge-CI `34001879727` sind erfolgreich. Die physische Windows-Abnahme ist bestanden, die physische Chrome-Abnahme aus P1b ausdrücklich verschoben. P2a ergänzt `routes`; die menschliche Referenzkurs-Abnahme und die letzte Feldstatus-Regression sind abgeschlossen. Maßgeblich sind [P1-Regeln](p1-rule-profile.md), [P2a-Routenvertrag](p2a-route-decisions.md), [Spieldesign](game-design.md), [Architektur](architecture.md), [Entscheidungen](decisions.md) und [Umsetzungsplan](implementation-plan.md).
...
Die echte GDScript-Suite `core` prüft Graph/Layout, Identität, Eingabeadapter, Start, Sperrgrenzen, Restart, Menü, Fokus und einmalige Ergebnisse mit einer injizierten Uhr. P1b ergänzt `integration` mit echter Szeneninstanziierung und Viewport-Eingaben. P2a ergänzt `routes` für Abschnittsports, Layout-/Anschlussdaten, alle vier Referenzrouten, die vollständige A–Z-QWERTZ-Zuordnung, Identität und flüchtige Abschnittszeiten/-fehler einschließlich Escape-/Fokus-/Restart-Lebenszyklus in einer echten Szeneninstanz. `integration` prüft zusätzlich die visuelle Statuspriorität: ein besuchtes, wieder erreichbares Feld behält Besuchsoberfläche/-marker und zeigt keinen Erreichbarkeitsrand. `all` führt immer alle vorhandenen Suites aus. Unbekannte/fehlende Suite, null ausgewählte Tests und Lade-/Assertion-/Laufzeitfehler dürfen nicht grün enden. Testanzahl und Ergebnis ausgeben; einen absichtlich fehlschlagenden Fall zur Runnerprüfung verwenden. Alte Tests nicht entfernen, um grün zu werden.
...
Der initiale Implementierungsstand `dcf0c87` bestand lokal unter Windows 11 Pro Build `26200` mit Godot `4.7.2.stable.official.ed1daf0bf` Import, `routes` mit **51 Assertions**, `all` mit **511 Assertions** sowie beide Release-Exporte. Nach dem technischen Review korrigiert `de9f8ee` die QWERTZ-Zuordnung und den Lebenszyklus der flüchtigen Abschnittsmessung. Die `routes`-Suite umfasst danach **84 Assertions**; `all` umfasst **544 Assertions**. GitHub-CI `34024281152` auf `de9f8ee` führte Import, die vollständige `all`-Suite einschließlich `integration` und `routes` sowie Windows- und Web-Releaseexport erfolgreich aus. Die neuen Regressionen prüfen alle A–Z-QWERTZ-Tasten einschließlich Y/Z/X/C sowie Fokusabbruch, laufendes Escape, post-finish Escape und Quick Restart; der Fokuspfad wird zusätzlich über eine tatsächlich instanziierte `PlayableCourseScene` geprüft.

Der native Windows-Release wurde auf dem initialen P2a-Implementierungsstand als sichtbares `Parkey`-Fenster bei 2560 × 1440 tatsächlich gestartet. Screenshots unter `build/evidence/windows-p2a-ready-foreground.png` und `build/evidence/windows-p2a-alpha-decision-synthetic.png` zeigen Bereitschaft und die erste Entscheidung nach synthetischem `A → Z → K`: aktuelles Feld, direkte Rückwege sowie `F` und `A` als erste Alternativen liegen ohne schwebende P1b-Callouts auf den Keycaps vor der Wahl. Die Sequenz und die Bildprüfung sind technische, synthetische Nachweise, keine physische Tastatur- oder menschliche Spielabnahme. Der erfolgreiche CI-Export auf `de9f8ee` ist ein Buildnachweis und ersetzt diesen Sichttest nicht.

Die eigentliche P2a-Nutzerabnahme umfasst vier vollständige Läufe über alle Routenkombinationen. Gemeldet wurden: `kurz/lang` **7.437.978 µs, 1 Fehler**, `kurz/kurz` **5.743.097 µs, 1 Fehler**, `lang/kurz` **6.903.868 µs, 0 Fehler**, `lang/lang` **8.705.214 µs, 2 Fehler**. Der Nutzer bewertet die Mechanik als funktional fehlerfrei und die Kamera für den aktuellen Referenzkurs als passend; bei komplexeren, nicht überwiegend gerade verlaufenden Strecken kann die Kameraführung später erneut angepasst werden. Die kleine Stichprobe reicht ausdrücklich nicht für eine allgemeine Tippbarkeits-/Ergonomieaussage; in diesen vier Läufen waren die kurzen Varianten jeweils schneller.

Aus der Abnahme folgt eine letzte Darstellungspräzisierung: Besuchsstatus hat visuell Vorrang vor Erreichbarkeit. Ein besuchtes Feld bleibt dunkler und erhält nur den Besuchsmarker; ein erneuter Erreichbarkeitsrand bzw. kombinierter Status wird unterdrückt. Die damalige `integration`-Suite prüfte Oberfläche, Marker und Rand; die neueste P2b-Nacharbeit entfernt die Symbole bei unverändertem Materialvorrang. GitHub-CI `34027294353` auf `b0270b4` bestätigte diese Nacharbeit mit erfolgreichem Import, vollständigen Tests sowie Windows- und Web-Releaseexport. Die ursprünglich gewünschte zweite Tippmethode bzw. möglichst zweite Person sowie eine separate grafische Web-Kamerawiederholung wurden vom Nutzer für diese P2a-Abnahme ausdrücklich als nicht mehr erforderlich bewertet. Sie werden nicht als durchgeführt behauptet. Die physische P1b-Chrome-Eingabeabnahme bleibt separat offen und ist kein P2a-Mergeblocker.

P2a ist fachlich abgenommen und merge-bereit. Die geprüften Entscheidungs-/Geometriebausteine können nach Merge als Grundlage für P3a dienen; die konkrete Annahme „länger = leichter/schneller“ ist nicht freigegeben.
...
Die Renderprüfung läuft separat im **Releaseexport**, unter Windows nativ und in Chrome über HTTP mit Compatibility. Der opt-in Prüfhelfer `tests/render_evidence.gd` spielt vier P2a-Kombinationen mit synthetischen Viewport-Ereignissen, realer Uhr und isolierten Ergebnissen. Acht Screenshots zeigen Bereitschaft, erste Entscheidung, besuchten Rückweg, Fehler, zweite Entscheidung, W und schmale Formate, Ziel und Debug. Rund 32 Sekunden Frameabstände werden nach Aufwärmen erfasst; Screenshot-Readback liegt außerhalb der Messfenster. Dies misst Anwendungsframeabstände, keine externe Scanout-/Eingabelatenz. Alle Pflichtzeichen und Statussignale werden zusätzlich anhand tatsächlicher Bilder kontrolliert. Prüfmethode, Bildgrößen, Hardware/Browser, Resultate und Grenzen stehen im [P2b-Bericht](p2b-visual-slice.md).

Die große visuelle Nacharbeit ergänzt Regressionen für symbolfreie Standard-Keycaps, explizite Barlow-Ressourcen, tatsächliche gedrehte Glyphenmaße innerhalb der Topflächen, beleuchtete Legenden ohne Outline und vollständige Projektion aller aktuellen/direkt benachbarten Zeichen an allen 30 Kamerapositionen. Hinzu kommen mikrosekundenfreie Standard-UI, eine vollständig passende Top-10-Tabelle und exakte interne Rangfolge bei gleicher Millisekundenanzeige; F3 bewahrt die Originalwerte. Kappenseiten, Normalen und versionierter Himmel werden in beiden Profilen geprüft. Die eigene Bildprüfung beurteilt zusätzlich Kappenform, integriertes Druckbild und Himmel-/Timer-Hierarchie gegen den lesbaren Original-Mock; Geometrietests allein gelten nicht als visueller Nachweis.
...
Der aktuelle Fidelity-Pass bewahrt die Regressionen für gebündelte Vertex-/Normaldaten, beide SSIL-Profile, normale Frequenzbegrenzung und Eingabe-/Kernparität. Die Figurenprüfung prüft den Sohlenkontakt unabhängig von der verkürzten Beinform und den modellierten Bob am bestehenden Reaktionspivot. Alle 30 Kamerapositionen behalten die vollständigen projizierten Glyphen des aktuellen Felds und sämtlicher direkten Nachbarn. Tatsächlich ausgeführt auf Windows/Godot 4.7.2: `presentation` 553, `integration` 238 und `all` 1103 Assertions ohne Fehler; Import und beide Releaseexporte Exit 0, fehlende/unbekannte Suite Exit 1.

Die aktuelle [#6-Anweisung](https://github.com/venomenon328/parkey/issues/6#issuecomment-5561815307) beendet die ausführliche Pacing-Untersuchung für diese Runde. Nach visuellen Änderungen genügen normale native Windows-1080p-/1440p- und Chrome-Release-Sanity-Läufe; beide vom Nutzer verwendeten Monitorfrequenzen werden berücksichtigt. Keine erneute PresentMon-/ETW-/Monitor-/Treiberforensik. D-028 und die normale Konfiguration bleiben erhalten. Finale Bilder, reale Werte, Iterationsbefunde gegen den externen Mock und `b236f17` sowie der reduzierte historische Ursachennachweis stehen im [P2b-Bericht](p2b-visual-slice.md). Eine zusätzliche opt-in Prüfkamera zeigt die reale Figur von vorn/hinten; diese Modellansichten sind keine normalen Gameplay-Screenshots.

Die subjektive Abnahme, der persönlich gespielte vollständige Windows-Lauf und die Beurteilung sichtbaren Stotterns/Bildreißens auf beiden Monitoren bleiben offen. Die ältere physische P1b-Chrome-Eingabeabnahme bleibt separat offen. Keine optische Scanout-Freigabe aus Anwendungsframes oder historischen ETW-Zeitstempeln ableiten.
