...
Stand: 2026-09-05. P0 ist technisch und manuell abgenommen. Das P1-Profil ist freigegeben und der P1a-Kern abgenommen/gemergt. P1b samt `integration`-Suite und Nacharbeit zu Review 5121935189 ist im Draft-PR #13 implementiert; CI/Re-Review und physische Plattformabnahme bleiben offen. Grundlage: [P1-Regelprofil](p1-rule-profile.md), [P1b-Integration](p1b-implementation.md), [Spieldesign](game-design.md), [Architektur](architecture.md), [Entscheidungen](decisions.md) und [Umsetzungsplan](implementation-plan.md).
...
Lokaler Nacharbeitsnachweis unter Windows 11 Pro 10.0.26200 mit Godot 4.7.2.stable.official.ed1daf0bf: `git diff --check`, Import, `integration` mit **156 Assertions / 0 Failures**, `all` mit **317 Assertions / 0 Failures** sowie beide Release-Exporte erfolgreich. CI wird nach dem Push mit dem Commit im Draft-PR #13 protokolliert; bis zum Re-Review und zur physischen Abnahme ist dies keine Mergeabnahme.

Die verpackten Zwischenstände wurden auf AMD Ryzen 7 5800X, NVIDIA GeForce RTX 3070, 32 GB RAM und 2560 × 1440 Desktopauflösung tatsächlich gestartet. Das Windows-Fenster lief bei 1280 × 720 mit Forward+; Chrome 152.0.7977.76 lud den Webexport über `http://127.0.0.1:8000/` mit Compatibility und HTTP 200 für HTML/JS/WASM/PCK. OS-synthetische Tastaturereignisse spielten in beiden Exporten obere und untere Route bis `FINISHED`/Fehler 0. Zusätzlich wurden Fehler/Restart, tatsächlicher Fenster-/Browserfokusverlust sowie der Nutzerpfad Klick ins Textfeld → Text/Backspace → Klick in freie Spielfläche → wirksames Backspace geprüft. Chrome wurde auf 960 × 620 verkleinert; Pflichttexte, Profil, aktuelle/rückwärtige Zeichen und Marker blieben in der visuellen Sanity-Prüfung lesbar. Ein lokaler Windows-Kameraclip über mehrere Schritte und einen Fehler liegt unversioniert unter `build/evidence/windows-camera-flow.mp4`; Screenshots liegen daneben.

**Offen:** Alle genannten Eingaben waren OS-synthetisch. Sie sind echte Paket-/Renderer-/Browser-/Fokusläufe, aber kein physischer Tastatur- oder Nutzer-Spieltest. Hardware-Y/Z und Shift, echtes Echo/Überlappen/Burstgefühl, händischer LineEdit-/Canvas-Fokus, Verständlichkeit der 200-ms-Grenze sowie menschliche Kamera-, Feldstatus-, Anschluss- und Routenlesbarkeit müssen auf Windows und Chrome anhand der Anleitung in [p1b-implementation.md](p1b-implementation.md) bestätigt werden. Re-Review und CI auf dem gepushten Head bleiben ebenfalls Abnahmegates; PR #13 bleibt Draft.
